...
namespace DataLayerRestaurant.Classes
{
    ...
    public class TablesRepositoryReader : ITablesRepositoryReader
    {
        ...
        public TablesRepositoryReader(IOptions<clsMySettings> settings)
        {
        ...
        public async Task<List<Table>> GetAllDataAsync(List<int> Ids)
        {
            ...
            {
                using (SqlCommand Command = new SqlCommand("Tables.SP_GetAllTablesByIds", Connection))
                {
                    ...
                    using (SqlDataReader reader = await Command.ExecuteReaderAsync())
                    {
                        ...
                        {
                            result.Add(TableMapper.ReaderToEntityResult(reader));
                        }
                    }
                }
            }
            ...
            using (SqlConnection Connection = new SqlConnection(_Settings.ConnectionString))
            {
                using (SqlCommand Command = new SqlCommand("Tables.SP_GetTablesAvailables", Connection))
                {
                    ...
                    using (SqlDataReader Reader = await Command.ExecuteReaderAsync())
                    {
                        while (await Reader.ReadAsync())
                        {
                            listTables.Add(TableMapper.ReaderToEntityResult(Reader));
                        }
                    }
                }
            }
        ...
        public async Task<Table?> GetDataAsync(int ID)
        {
            ...
            using (SqlConnection Connection = new SqlConnection(_Settings.ConnectionString))
            {
                using (SqlCommand Command = new SqlCommand("Tables.SP_GetTableByID", Connection))
                {
                    ...
                    using (SqlDataReader Reader = await Command.ExecuteReaderAsync())
                    {
                        table = (await Reader.ReadAsync()) ? TableMapper.ReaderToEntityResult(Reader) : null;
                    }
                }
            }
    ...
    public class TablesRepositoryWriter : ITablesRepositoryWriter
    {

        private readonly clsMySettings _Settings;
        public TablesRepositoryWriter(IOptions<clsMySettings> settings)
        {
            _Settings = settings.Value;
        }
            ...
            using (SqlConnection Connection = new SqlConnection(_Settings.ConnectionString))
            {
                using (SqlCommand Command = new SqlCommand("Tables.SP_AddTable", Connection))
                {
                    ...
                    using (SqlDataReader Reader = await Command.ExecuteReaderAsync())
                    {
                        ...
                        {
                            create = TableMapper.ReaderToEntityResult(Reader);
                        }
                    }
                }
            }
            ...
            using (SqlConnection Connection = new SqlConnection(_Settings.ConnectionString))
            {
                using (SqlCommand Command = new SqlCommand("Tables.SP_UpdateTable", Connection))
                {
                    ...
                    using (SqlDataReader Reader = await Command.ExecuteReaderAsync())
                    {
                        if (await Reader.ReadAsync())
                        {
                            Update = (TableMapper.ReaderToEntityResult(Reader));
                        }
                    }
                }
            }
