...
namespace APILayer.Controllers
{
    ...
    public class APIOrders : BaseController
    {
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<ActionResult<ApiResponse<DTOOrders?>>> GetOrderByID(int id)
        {
            try
            {
                if (id <= 0)
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<ActionResult<ApiResponse<DTOOrders>>> AddNewOrder([FromBody] DTOOrders dto)
        {
            try
            {
                ...
                return CreatedAtRoute(
                    "GetOrder",
                    new { ID = this._businessOrders.DTOOrders!.ID },
                    _businessOrders.DTOOrders
                );
            }
            catch (Exception ex)
            {
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<ActionResult<ApiResponse<DTOOrders>>> UpdateOrder([FromBody] DTOOrders dto)
        {
            try
            {
                if (dto == null || dto.StatusOrderID <= 0 || dto.TableID <= 0 || dto.EmployerID <= 0 )
                ...
                _businessOrders.DTOOrders = await _businessOrders.GetOrderAsync(dto.ID);
